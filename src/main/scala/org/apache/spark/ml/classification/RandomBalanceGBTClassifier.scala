...
import org.apache.spark.ml.tree.impl.RandomBalanceGradientBoostedTrees
 ...
 */
@Since("1.4.0")
class RandomBalanceGBTClassifier @Since("1.4.0") (
    @Since("1.4.0") override val uid: String)
  extends ProbabilisticClassifier[Vector, RandomBalanceGBTClassifier, GBTClassificationModel]
  ...
  override protected def train(
    ...
    val (baseLearners, learnerWeights) = if (withValidation) {
      RandomBalanceGradientBoostedTrees.runWithValidation(trainDataset, validationDataset, boostingStrategy,
        $(seed), $(featureSubsetStrategy), Some(instr))
    } else {
      RandomBalanceGradientBoostedTrees.run(trainDataset, boostingStrategy, $(seed), $(featureSubsetStrategy),
        Some(instr))
    }
  ...
  @Since("1.4.1")
  override def copy(extra: ParamMap): RandomBalanceGBTClassifier = defaultCopy(extra)
}

@Since("1.4.0")
object RandomBalanceGBTClassifier extends DefaultParamsReadable[RandomBalanceGBTClassifier] {
  ...
  @Since("2.0.0")
  override def load(path: String): RandomBalanceGBTClassifier = super.load(path)
