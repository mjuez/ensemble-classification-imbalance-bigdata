...
private[spark] object RandomBalanceGradientBoostedTrees extends Logging {
  ...
  def run(
    ...
    algo match {
      case OldAlgo.Regression =>
        RandomBalanceGradientBoostedTrees.boost(input, input, boostingStrategy, validate = false,
      ...
      case OldAlgo.Classification =>
        ...
        RandomBalanceGradientBoostedTrees.boost(remappedInput, remappedInput, boostingStrategy, validate = false,
          seed, featureSubsetStrategy, instr)
      case _ =>
  ...
  def runWithValidation(
    ...
    algo match {
      case OldAlgo.Regression =>
        RandomBalanceGradientBoostedTrees.boost(input, validationInput, boostingStrategy,
      ...
      case OldAlgo.Classification =>
        ...
        val remappedValidationInput = validationInput.map(
          x => Instance((x.label * 2) - 1, x.weight, x.features))
        RandomBalanceGradientBoostedTrees.boost(remappedInput, remappedValidationInput, boostingStrategy,
          validate = true, seed, featureSubsetStrategy, instr)
      case _ =>
  ...
  private def imbalanceSampling(
    ...
    val size = minNum + majNum

    val rng = new Random()
    rng.setSeed(seed)
    val newMajNum = rng.nextInt(size.toInt - 3) + 2.0 // random between 2 and size-2 (inclusive)
    val newMinNum = size - newMajNum

    if (newMajNum != newMinNum) {
      val newMajFrac = newMajNum / majNum.toDouble
      val newMinFrac = newMinNum / minNum.toDouble

      val minority = data.filter(p => p.datum.label == minClass)
      val majority = data.filter(p => p.datum.label == majClass)

      if (newMinFrac > 1){
        val newMajo = majority.sample(false, newMajFrac, rng.nextLong)
        val newMino = minority.sample(true, newMinFrac, rng.nextLong)
        newMajo.union(newMino)
      }else{
        val newMino = minority.sample(false, newMinFrac, rng.nextLong)
        val newMajo = majority.sample(true, newMajFrac, rng.nextLong)
