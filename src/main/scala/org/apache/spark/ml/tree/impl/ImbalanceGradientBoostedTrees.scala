...
private[spark] object ImbalanceGradientBoostedTrees extends Logging {

  /**
   * Method to train a gradient boosting model
   * @param input Training dataset: RDD of `Instance`.
   * @param randomUnderSampling whether RUS or ROS is applied
   * @param percSampling percentage of sampling in RUS/ROS
  ...
  def run(
    ...
    algo match {
      case OldAlgo.Regression =>
        ImbalanceGradientBoostedTrees.boost(input, input, boostingStrategy, validate = false,
          seed, featureSubsetStrategy, randomUnderSampling, percSampling, instr)
      case OldAlgo.Classification =>
        // Map labels to -1, +1 so binary classification can be treated as regression.
        val remappedInput = input.map(x => Instance((x.label * 2) - 1, x.weight, x.features))
        ImbalanceGradientBoostedTrees.boost(remappedInput, remappedInput, boostingStrategy, validate = false,
          seed, featureSubsetStrategy, randomUnderSampling, percSampling, instr)
      case _ =>
  ...
  def runWithValidation(
    ...
    algo match {
      case OldAlgo.Regression =>
        ImbalanceGradientBoostedTrees.boost(input, validationInput, boostingStrategy,
          validate = true, seed, featureSubsetStrategy, randomUnderSampling, percSampling, instr)
      case OldAlgo.Classification =>
        // Map labels to -1, +1 so binary classification can be treated as regression.
        ...
        val remappedValidationInput = validationInput.map(
          x => Instance((x.label * 2) - 1, x.weight, x.features))
        ImbalanceGradientBoostedTrees.boost(remappedInput, remappedValidationInput, boostingStrategy,
          validate = true, seed, featureSubsetStrategy,randomUnderSampling, percSampling, instr)
      case _ =>
  ...
  def boost(
    ...
    while (m < numIterations && !doneLearning) {
      ...
      val bagged = imbalanceSampling(treePoints.zip(labelWithCounts)
        ...
        }, seed, randomUnderSampling, percSampling)
  ...
  private def imbalanceSampling(
      data: RDD[BaggedPoint[TreePoint]],
      seed: Long,
      randomUnderSampling: Boolean,
      percSampling: Int): RDD[BaggedPoint[TreePoint]] = {

    // Find out the minority class.
    val classes = data.map(e => e.datum.label).groupBy(p => p).mapValues(_.size).sortBy(_._2).collect

    val (minClass, minNum) = classes(0)
    val (majClass, majNum) = classes(classes.size - 1)

    val minority = data.filter(p => p.datum.label == minClass)

    // Perc. unders. equal to 0 means to balance minority and majority classes
    var fraction = 0.0
    
    // RUS or ROS
    if (randomUnderSampling) {
      val notMajority = data.filter(p => p.datum.label != majClass)
      val majority = data.filter(p => p.datum.label == majClass)

      if (percSampling == 0)
        fraction = minNum.toFloat / majNum
      else
        fraction = percSampling.toFloat / 100
        
      notMajority.union(majority.sample(false, fraction, seed))
    }
    else {
      if (percSampling == 0)
      ...
      data.union(minority.sample(true, fraction, seed))
    }
  }
}
